...
// OVERWRITES the signed `x-cmux-authorization` header on every request like the real
...
if (inject && !(/^[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+$/.test(routeToken))) {
  console.error("--route-token must be a signed JWT");
  process.exit(2);
}
